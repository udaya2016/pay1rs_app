...
const Welcome = ({ navigation, route }) => {
  ...
  useEffect(() => {
    const apiUrl = 'https://pay1rs.com/api/getAvailableBalance';
  ...
  return (
    <StyledContainer>
      ...
      <InnerContainer>
        ...
        <EwalletTitle>
          E-Wallet: {eWallet ? <Text>₹{parseFloat(eWallet).toFixed(2)}</Text> : <Text>₹ {zerobalance.toFixed(2)}</Text>}
        </EwalletTitle>
        <View style={styles.container}>
          <Button  onPress={() => {
                navigation.navigate('Mobile', { ...route });
              }}
          ...
          <Button  onPress={() => {
                navigation.navigate('Postpaid', { ...route });
              }}
            icon={<Icon name="mobile" type="font-awesome" color="#ffffff" />}
            title="Postpaid"
            buttonStyle={styles.button}
          ...
          <View style={styles.spacing} />
          <Button onPress={() => {
                navigation.navigate('Electricity', { ...route });
              }}
