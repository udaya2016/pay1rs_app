...
const Postpaid = ({ navigation, route }) => {
  ...
  const handleRechargeApi = (formData, setSubmitting) => {
    handleMessage(null);
    const url = 'https://pay1rs.com/api/postpaidFetchBill';
    ...
    axios
      ...
      .then((response) => {
        ...
        console.log(data);
        if (status != 'SUCCESS') {
          handleMessage("Invaild Details", status);
        } 
        else {
          navigation.navigate('PostpaidModal', { ...data });
          console.log(...data);
        }
        setSubmitting(false);
      })
    ...
    <KeyboardAvoidingWrapper>
      <StyledContainer>
        ...
        <InnerContainer>
          ...
          <SubTitle>Postpaid Recharge</SubTitle>

          <CarouselSlider />
          <Formik
            initialValues={{ mobile: '', network: '' }}
            onSubmit={(values, { setSubmitting }) => {
              if (values.mobile == '' || values.network == '') {
